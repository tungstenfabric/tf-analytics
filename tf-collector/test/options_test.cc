...
#include "base/test/task_test_util.h"
#include "tf-collector/options.h"
...
TEST_F(OptionsTest, DefaultConfFile) {
    ...
    char *argv[argc];
    char argv_0[] = "options_test";
    char argv_1[] = "--conf_file=src/tf-analytics/tf-collector/tf-collector.conf";
    argv[0] = argv_0;
    argv[1] = argv_1;

    options_.Parse(evm_, argc, argv);
    vector<string> passed_conf_files;
    passed_conf_files.push_back("src/tf-analytics/tf-collector/tf-collector.conf");
    ...
    EXPECT_EQ(options_.log_category(), "");
    EXPECT_EQ(options_.log_disable(), false);
    EXPECT_EQ(options_.log_file(), "/var/log/contrail/tf-collector.log");
    EXPECT_EQ(options_.log_files_count(), 10);
    EXPECT_EQ(options_.log_file_size(), 1024*1024);
    EXPECT_EQ(options_.log_level(), "SYS_NOTICE");
...
TEST_F(OptionsTest, MultitokenVector) {
    ...
    char argv_3[] = "--conf_file=src/tf-analytics/tf-collector/tf-collector.conf"
                    " src/tf-analytics/tf-collector/tf-database.conf";
    char argv_4[] = "--conf_file=src/tf-analytics/tf-collector/test-conf.conf";
    argv[0] = argv_0;
    argv[1] = argv_1;
    argv[2] = argv_2;
    argv[3] = argv_3;
    ...
    option_file_list.push_back("src/tf-analytics/tf-collector/tf-collector.conf");
    option_file_list.push_back("src/tf-analytics/tf-collector/tf-database.conf");
    option_file_list.push_back("src/tf-analytics/tf-collector/test-conf.conf");
    TASK_UTIL_EXPECT_VECTOR_EQ(options_.config_file(),
                     option_file_list);
}
