    ...
    def verify_vm_uve(self, vm_id, num_vm_ifs, msg_count, opserver_port=None):
        ...
        if res == {}:
            ...
            for i in range(num_vm_ifs):
                ...
                self._logger.info('Aadarsh: Value of evm_if_name is %s',evm_if_name)
                self._logger.info('Aadarsh: Value of avm_if_name is %s',avm_if_name)
                self._logger.info('Aadarsh: Near assert avm_if_name = vm_if_dict')
                assert avm_if_name == evm_if_name
            self._logger.info('Aadarsh: returning True after loop')
            return True
    # end verify_uve_vm
